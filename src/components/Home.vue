<template>
  <section class="Home">
    <div ref="div" class="Scene"></div>
    <h1 class="Title Title--first">Acker'<br />Prod</h1>
    <p class="Title Title--blue">Acker'<br />Prod</p>
    <p class="Title Title--red">Acker'<br />Prod</p>
    <button class="Button">version résumée</button>
    <p class="Description Description--first">web and 3D experience <br> creation studio</p>
    <p class="Description Description--blue">web and 3D experience <br> creation studio</p>
    <p class="Description Description--red">web and 3D experience <br> creation studio</p>
  </section>
</template>

<script setup lang="ts">
import { ref, onMounted } from "vue";
import { Engine } from "../three/engine"

let engine;
const div = ref();

onMounted(() => {
  engine = new Engine(div.value)
})
</script>

<style scoped>
.Home {
  height: 100vh;
  width: 100vw;
  color: white;
  position: relative;
}

.Scene {
  position: fixed;
  top: 0;
  left: 0;
}

.Title {
  position: fixed;

  margin: 0;
  font-family: 'Mewatonia';
  font-size: 64px;
  line-height: 94%;
}

.Title--first {
  top: 50px;
  left: 50px;
  z-index: 1;
}
.Title--red {
  top: 52px;
  left: 52px;
  color: rgb(255, 48, 21);
}
.Title--blue {
  top: 48px;
  left: 48px;
  color: rgb(0, 255, 213);
}

.Button {
  position: fixed;
  top: 50px;
  right: 50px;
  background: none;
  padding: 15px;
  color: white;
  font-family: 'Mewatonia';
  border: 4px white solid;
  font-size: 1.3rem;
  cursor: pointer;
}

.Description {
  position: fixed;
  margin: 0;
  font-family: 'News';
  font-style: italic;
  font-size: 36px;
}

.Description--first {
  bottom: 50px;
  right: 50px;
  z-index: 1;
}
.Description--blue {
  bottom: 51.6px;
  right: 51.2px;
  color: rgb(0, 255, 213);
}
.Description--red {
  bottom: 49px;
  right: 48px;
  color: rgb(255, 48, 21);
}

.Button {
  font-family: 'News';
  font-style: italic;
}
</style>
